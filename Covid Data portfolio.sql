...
-- Total deaths, vaccinations, cases, population for the World

SELECT deaths.population, MAX(deaths.total_cases), MAX(deaths.total_deaths), MAX(vacs.people_fully_vaccinated)
FROM covid.deaths
JOIN covid.vacs
ON deaths.location = vacs.location
and deaths.date = vacs.date
WHERE deaths.location LIKE 'World'
